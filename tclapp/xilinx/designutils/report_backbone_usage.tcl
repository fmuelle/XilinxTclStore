...
proc ::tclapp::xilinx::designutils::report_backbone_usage::report_backbone_usage {nets {table {false}}} {
	...
	foreach net $nets {
		...
		set IBUFDS_Ipin_net [get_nets -quiet -of [get_pins -quiet -of [get_cells -quiet -of [get_pins -leaf -of $net -filter DIRECTION==OUT] -filter LIB_CELL=~IBUFDS*] -filter REF_PIN_NAME=~I]] 
